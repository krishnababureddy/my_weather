...
import '/viewmodels/get_weekly_forecast_provider.dart';
...
class WeeklyForecastView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final weeklyForecastAsync = ref.watch(weeklyForecastProvider);

    return weeklyForecastAsync.when(
      data: (response) {
        // Check for error
        if (response.errorMessage != null) {
          return Center(child: Text(response.errorMessage!));
        }

        final weatherData = response.weeklyweather!;
